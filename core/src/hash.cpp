...
constexpr std::uint32_t Ror(std::uint32_t x, unsigned n) noexcept {
    return (x >> n) | (x << (32 - n));
}
...
void Sha256::Update(std::span<const std::uint8_t> data) noexcept {
    ...
    if (off < data.size()) {
        const std::size_t rest = data.size() - off;
        std::memcpy(buf_.data(), data.data() + off, rest);
        buf_len_ = rest;
    }
}
...
Sha256::Digest Sha256::Finish() noexcept {
    ...
    // Update() advanced total_bits_; restore so the length field is correct.
    total_bits_ = bits;
    pad = 0x00;
    while (buf_len_ != 56) {
        Update(std::span<const std::uint8_t>(&pad, 1));
    ...
    for (int i = 0; i < 8; ++i)
        len[static_cast<std::size_t>(i)] =
            static_cast<std::uint8_t>((bits >> ((7 - i) * 8)) & 0xFFU);
    Update(len);
...
Sha256::Digest Sha256::Of(std::span<const std::uint8_t> data) noexcept {
    Sha256 s;
    s.Update(data);
    return s.Finish();
}
